...
export async function itentifyAddress(address, id, provider, user) {
    const contractCode = await provider.getCode(address);
    if (contractCode === '0x')
        return { type: 'EOA', address, inputId: id };
    ...
    const [isERC721, hasMetadata, isERC1155, isERC1155Metadata, owner, name, symbol, decimals, totalSupply, tokenURI, erc1155Uri] = await multicall.tryAggregate.staticCall(false, calls);
    try {
        if (isERC721.success && nftInterface.decodeFunctionResult('supportsInterface', isERC721.returnData)[0] === true) {
            if (owner.success === false || nftInterface.decodeFunctionResult('ownerOf', owner.returnData)[0] === ZeroAddress)
        ...
        if (isERC1155.success && nftInterface.decodeFunctionResult('supportsInterface', isERC1155.returnData)[0] === true) {
            ...
            const uri = erc1155Uri.success && isERC1155Metadata.success && erc1155Interface.decodeFunctionResult('supportsInterface', isERC1155Metadata.returnData)[0] === true ? erc1155Interface.decodeFunctionResult('uri', erc1155Uri.returnData)[0] : undefined;
            return {
                type: 'ERC1155',
                inputId: id,
                id,
...
//# sourceMappingURL=identifyTokens.js.map